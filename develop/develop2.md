...
- 目前只保留小步推进边界：AxVisor 和 OVMF 只做日志增强或单点适配；一旦需要大规模逻辑改动、复杂平台补齐或 `sudo` 权限，就停止并记录边界。
...
- 候选的最小改动点是：
  ...
  - `tgoskits/components/axvisor/src/vmm/vcpus.rs`：在 `AxVCpuExitReason::IoWrite` 里专门打印 `0x402` 的单字节写出。
...
## Step 9：实现最小 fw_cfg 设备并推进到 DMA 语义校正

### 做了什么
- 在 `tgoskits/components/axvm/src/vm.rs` 中加入了最小 fw_cfg 状态和端口/DMA 处理。
- 在 `tgoskits/components/x86_vcpu/src/vmx/vcpu.rs` 中把 fw_cfg 端口加入 VMX I/O bitmap 拦截范围。
- 把 OVMF PEI 所需的最小 fw_cfg 项准备出来：
  - signature `QEMU`
  - interface version `FW_CFG_F_DMA`
  - SMP CPU count
  - `etc/e820`
  - file directory

### 看到什么日志 / 观察点
- 目前这一步主要还是代码适配和编译验证，没有新的 smoke test 日志。
- 我随后重新核对了 EDK2 的 fw_cfg 读实现，确认普通 `QemuFwCfgRead16/32()` 直接把字节读入本地整数，调用方再解释，因此普通 item 的值不需要在 AxVisor 侧额外 swap。
- 我同时确认了 `QemuFwCfgSelectItem()` 直接 `IoWrite16(FW_CFG_IO_SELECTOR, item)`，所以 selector 端口按原始 16 位值处理是对的。

### 判断出现什么问题
- 当前最需要小心的地方是 DMA descriptor 地址端口 `0x514/0x518`：这里不是普通 item 读写，而是 OVMF 用 `IoWrite32` 连续写两次，AxVisor 必须按 EDK2 的 `SwapBytes32()` 语义组装成 64 位 descriptor 地址。
- 之前的第一版实现把这些写操作按普通值保存过一轮，后来已经改成按 `from_be`/`to_be` 方式还原，使其更贴近 EDK2 的实际 DMA 写法。
- 另一处需要保守处理的是 `FW_CFG_DMA_CTL_ERROR`：当前 handler 已能写回 status，但仍只做了最小语义，后续 smoke test 若暴露问题，再继续收紧。

### 根据 `docs/OVMF-Boot-Overview.md` 判断处于哪个阶段
- 仍然是 **PEI**。
- 当前是在 PEI 里为 `PlatformPei` 提供最小 fw_cfg 基础设施，而不是进入 DXE 或完整平台模型。

### 做了什么改动
- `tgoskits/components/axvm/src/vm.rs`：新增 `FwCfgState` 和最小 fw_cfg item 生成逻辑。
- `tgoskits/components/axvm/src/vm.rs`：新增 `handle_fw_cfg_io_read()`、`handle_fw_cfg_io_write()`、`handle_fw_cfg_dma()`、guest 内存读写助手。
- `tgoskits/components/x86_vcpu/src/vmx/vcpu.rs`：拦截 `0x510..0x511`、`0x514..0x51b`。
- `tgoskits/components/axvm/src/vm.rs`：`FW_CFG_ITEM_INTERFACE_VERSION` 保持按小端字节序存放，匹配 OVMF 直接读入本地 `UINT32` 后比较 feature bit 的用法。
- `tgoskits/components/axvm/src/vm.rs`：DMA 地址端口的组装逻辑改为按 EDK2 `SwapBytes32()` 的实际写法还原。
- `cargo check --manifest-path /home/ssdns/work/axvisor-uefi/tgoskits/Cargo.toml -p axvm -p x86_vcpu` 已通过。

### 下一步最小动作是什么
- 还需要跑一次 OVMF smoke test，确认是否已经跨过旧的 `PlatformPei.dll` / `InternalQemuFwCfgDmaBytes()` `#BR`。
- 如果仍然失败，再记录新的日志与具体停点，判断是 DMA 语义、status 回写还是缺少某个 fw_cfg item。
- `develop2.md` 现在已经补写完成，但还需要在后续运行结果出来后继续追加记录。

## Step 10：绕过 fw_cfg string I/O 后跨过 PlatformPei 的 fw_cfg cache

### 做了什么
- 运行 smoke test 后发现新停点不是旧的 fw_cfg DMA `#BR`，而是更早的 VMX string I/O：`rep insb` 读 fw_cfg data port `0x511`。
- 按 debugcon 的同类最小策略修改 OVMF PEI probe 路径，不在 AxVisor 里扩大完整 string I/O 支持面。
- 重新构建 DEBUG OVMF，并把新 `OVMF_CODE.fd` 写入 rootfs 后校验 SHA256 一致。
- 重新运行 AxVisor OVMF smoke test。
...
- 第一次运行新 AxVisor 后停在：
  ...
  - `rcx: 0x4`
  - `rdx: 0x511`
- 修改并重建 OVMF 后，`0x511` string I/O 停点消失，看到：
  - `OVMF debugcon: QemuFwCfgProbe: Supported 1, DMA 1`
...
### 判断出现什么问题
- 这次已经明确跨过了旧的 `PlatformPei` / `QemuFwCfgPei` DMA 卡点；fw_cfg cache、`etc/e820`、SMP CPU count 都已被 OVMF 成功读取。
- `PlatformPei` 已经推进到 `PublishPeiMemory()` / `PlatformQemuInitializeRam()` 附近，并开始根据 e820 建 HOB。
- 新问题不是 fw_cfg 格式本身，而是 MTRR 初始状态不满足 OVMF 假设：`MemDetect.c:1343-1345` 期望 `MtrrDefType` 为 WB、enabled bit 为 0、E bit 为 1；当前 BIT10 已经为 1，触发 ASSERT。
- 这说明下一步应转向 MTRR/MSR 初始状态或 OVMF 对 AxVisor 当前 MTRR 暴露方式的单点适配，而不是继续补 fw_cfg item。

### 根据 `docs/OVMF-Boot-Overview.md` 判断处于哪个阶段
- 仍然是 **PEI**。
- 但已经跨过 `PlatformPei` 的 fw_cfg cache 初始化，推进到 PEI 的永久内存发布 / RAM 初始化路径。
- 尚未进入 DXE。

### 做了什么改动
- `edk2/OvmfPkg/Library/QemuFwCfgLib/QemuFwCfgPei.c`：在 `QemuFwCfgProbe()` 中把 signature 和 interface version 的 `IoReadFifo8(FW_CFG_IO_DATA, ...)` 改为逐字节 `IoRead8(FW_CFG_IO_DATA)`，避免 AxVisor 当前不支持的 `rep insb`。
- `tgoskits/components/axvm/src/vm.rs`：保留最小 fw_cfg 实现，当前 smoke test 已验证 `QemuFwCfgInitCache Pass!!!`。
- `tgoskits/components/x86_vcpu/src/vmx/vcpu.rs`：保留 `0x510..0x511`、`0x514..0x51b` 的 I/O bitmap 拦截。
- rootfs：已用新构建的 `/home/ssdns/work/axvisor-uefi/Build/OvmfX64/DEBUG_GCC/FV/OVMF_CODE.fd` 覆盖 `/guest/ovmf/OVMF_CODE.fd`，SHA256 校验一致。

### 下一步最小动作是什么
- 不再继续扩展 fw_cfg item。
- 先检查 OVMF SEC `SecMtrrSetup()` 和 AxVisor VMX MSR/MTRR 透传状态，确认为什么 `IA32_MTRR_DEF_TYPE` 的 BIT10 在 `MemDetect.c:1344` 前已经为 1。
- 如果能用 AxVisor 侧最小 MSR/MTRR 初始化或拦截修正，就做单点适配；如果需要完整 MTRR 虚拟化，则停止并记录边界。
